...
$(document).ready(function(){
	...
	var inputFormValidate=$("#input_form").validate({
		rules:{
			...
			curpwd:{required:true,checkOldPw:true}
		},
		messages: {
			pwd1:{
				equalTo: "前后密码不一致！"
...
jQuery.validator.addMethod("checkOldPw",function(value,element){
	...
	var url="${ctx }/ic/user!checkOldPwd.action";
	var id=$("#curuserid").val();
	var curpwd=$("input[name='curpwd']").val();
	var oldpwd=value;
	var result;
	$.ajax({
		url:url,
		data:{"curuserid":id,"curpwd":curpwd},
		type:"post",
		datatype:"json",
...
<body>
	...
	<input type="hidden" id="curuserid" name="id" value="${curUser.id }">
