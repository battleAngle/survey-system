									...
									<%--是非题 --%>
																	...
																	<c:forEach items="${en.anCheckboxs }" var="anCk" varStatus="iStatus">
																		<c:choose>
																			<c:when test="${anCk.quItemId eq quItem.id }">
																			    <c:choose>
																			     <c:when test="${quItem.noCheckBox == 1 }">
																			     <li><input type="checkbox" name="tag_qu_${en.quType }_${en.id }_${quItem.id }"  value="${quItem.id }"  checked="checked" style="display:none"/></li>
																			     </c:when>
																			     <c:otherwise>
																			...
																			<c:when test="${(fn:length(en.anCheckboxs) eq iStatus.count ) && (isBreak eq 0) }">
																				<c:choose>
																			     <c:when test="${quItem.noCheckBox == 1 }">
																			     <li><input type="checkbox" name="tag_qu_${en.quType }_${en.id }_${quItem.id }"  value="${quItem.id }"  style="display:none"/></li>
																			     </c:when>
																			     <c:otherwise>
									 ...
									 <%--排序 --%>
										<c:when test="${en.quType eq 'ORDERQU' }">
											<ul class="r-qu-body-ul1 r-qu-body-ul3">
										<!-- 	quOrderbysgetAnOrders -->
												<c:forEach items="${en.anOrders}" var="anOrder">
												
												 <c:forEach items="${en.quOrderbys }" var="quOrderby">
												 <c:if test="${anOrder.quRowId eq quOrderby.id }">
												 <li>
												 <ul class="quItem-ul">
												 <li>${quOrderby.optionName}</li>
												 </ul>
												 </li>
												 </c:if>
											...
											<table class="qusTable" width="90%" >
												...
												<c:forEach items="${en.quScores }" var="quScore">
													<tr>
														...
														<td><input type="text"  name="item_qu_${en.quType }_${en.id }_${quScore.id }" value="${enCol }" style="margin-left:35%"/> </td>
													</tr>
												</c:forEach>
											</table>
											<c:forEach items="${en.anScores }" var="enAn">
												<script type="text/javascript">
												/*[value='${enAn.answserScore }']  */
													$("input[name='item_qu_${en.quType }_${enAn.quId }_${enAn.quRowId }']").val('${enAn.answserScore }');
												</script>
											</c:forEach>
										</c:when>
											...
											<table class="qusTable" width="90%" >
												...
												<c:forEach items="${en.rows }" var="enRow">
													<tr>
														...
														<c:forEach items="${en.columns }" var="enCol">
															<td align="center"><span  class="item_qu_${en.quType }_${en.id }_${enRow.id }_${enCol.id}" ></span> </td>
														</c:forEach>
													</tr>
												</c:forEach>
											</table>
											 <c:forEach items="${en.anChenScores }" var="enAn">
												<script type="text/javascript">
												  	$(".item_qu_${en.quType }_${enAn.quId }_${enAn.quRowId }_${enAn.quColId}").text("${enAn.answserScore}");
												</script>
											</c:forEach> 
										</c:when>
										<%--矩阵单选题 --%>
