...
public class SurveyDirectoryManagerImpl extends BaseServiceImpl<SurveyDirectory, String> implements SurveyDirectoryManager {
	...
	public SurveyDirectory getSurveyBySid(String sid) {
		...
		getSurveyUsers(surveyDirectory.getId(),surveyDirectory);
	...
	public void getSurveyUsers(String id,SurveyDirectory directory) {
		
		 //根据id获得SurveyUser的集合
		 List<SurveyUser> surveyusers=surveyuserdao.findBy("directory_id", id);
		 
		 if(surveyusers != null){
			 directory.setSurveyUsers(surveyusers);
		 }else{
			 surveyusers=new ArrayList<SurveyUser>();
			 directory.setSurveyUsers(surveyusers);
		 }
...
//		directory.setSurveyDetailId(detail.getId());
	...
	private SurveyDirectory buildCopyObj(String fromBankId, String surveyName,String tag) {
		...
		surveyDirectory.getSurveyDetail().setSurveyNote(directory.getSurveyDetail().getSurveyNote());
		return surveyDirectory;
	}

	@Override
	public List<SurveyDirectory> findByName(String name) {
		
		Criterion cri1=Restrictions.eq("surveyName", name);
		 List<SurveyDirectory> directorys=surveyDirectoryDao.find(cri1);
		return directorys;
	}
