...
@AllowedMethods({"ajaxSave","ajaxDelete","ajaxdeleteSubentry"})
public class QuCheckboxAction extends ActionSupport{
	...
	public String ajaxdeleteSubentry() throws Exception{
		 HttpServletRequest request=Struts2Utils.getRequest();
	     HttpServletResponse response=Struts2Utils.getResponse();
		 try{
				...
				for (String string : idArray) {
					QuCheckbox quCheckbox=quCheckboxManager.get(string);
					if(quCheckbox != null){
						quCheckboxManager.ajaxDelete(string);
					}
				}
	...
	private Question ajaxBuildSaveOption(HttpServletRequest request) throws UnsupportedEncodingException {
		...
		String isSubentry=request.getParameter("isSubentry");
...
//			quTitle =quTitle.replace("href=\"", "href=\""+projectName);
		...
		entity.setIsSubentry(isSubentry);
		...
		for (String key : optionNameMap.keySet()) {
			...
			String noCheckBoxString=request.getParameter("noCheckBox_"+key);
...
//			quRadio.setOptionTitle(key);
			...
			Integer noCheckBox=(noCheckBoxString == null || "".equals(noCheckBoxString))?0:Integer.parseInt(noCheckBoxString);
			quCheckbox.setNoCheckBox(noCheckBox);
